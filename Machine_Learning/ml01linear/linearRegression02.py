...
x_train, x_test, y_train, y_test = \
    train_test_split(x, y, train_size=0.8, random_state=seed)

from sklearn.linear_model import LinearRegression
model = LinearRegression()  # 모델 객체 생성

model.fit(x_train, y_train)     # 훈련 데이터로 피팅(학습)
...
print('score 함수는 결정 개수를 구해주는 함수')
print('결정 개수 : %.3f' % model.score(x_test, y_test))
