...
# subplot(x축 개수, y축 개수, 인덱스 개수)
num_rows = 4    # 행 수
num_cols = 4    # 열 수
num_images = num_cols * num_rows    # 그리고자 하는 이미지 개수
...
def plot_image(i, prediction_array, true_label, img):
    ...
    pred = class_names[prediction_label]    # 예측 값
    prob = 100.0 * np.max(prediction_array[i])  # 제일 큰 확률
    answer = class_names[true_label[i]]
    plt.xlabel("{} {:6.2f}% ({})".format(pred, prob, answer), color=mycolor)    # (예측, 확률, 정답)
# end def plot_image
...
# 0 번째 test 데이터를 이용한 이미지와 확률 값에 대한 그래프
idx = 0  # 이미지 색인 번호
